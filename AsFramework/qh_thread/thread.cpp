#include "common/AsFramework/qh_thread/thread.h"
#include <errno.h>
#include <strings.h>
#include <signal.h>
#include <sys/prctl.h>
...
void QH_THREAD::CThread::set_thread_name(const char* thread_name) {
    if (thread_name) {
        m_thread_name_ = std::string(thread_name);
    }
}
...
void *QH_THREAD::CThread::private_thread_func(void *this_ptr) {
    ...
    if (!(cur_thread->m_thread_name_).empty()) {
        prctl(PR_SET_NAME, (cur_thread->m_thread_name_).c_str());
    }
...
void QH_THREAD::CWorkerThread::SetThreadName(const char* thread_name) {
    if (thread_name) {
        m_thread_name_ = std::string(thread_name);
    }
}
