...
// #include "json/json.h"
#include "common/dependlibs/CentOS_jsoncpp/include/jsoncpp/json/json.h"
...
public:
	...
	static bool WriteJsonToUnstyledString( const Json::Value &jvRoot, std::string& strJson)
	{
		try
		{
	...
	static bool WriteJsonToUnstyledString2( const Json::Value &jvRoot, std::string& strJson)
	{
		try
		{
			...
			strJson = Json::writeString(wbuilder, jvRoot);
			return true;
		}
		catch(...) { return false;}
	}
	...
	static bool GetJsonValueBool(const char* lpKey, const Json::Value& jvRoot, bool bDefault = false)
	{
		try
		{
			if(jvRoot.isNull() || !jvRoot.isObject())		return bDefault;
			if(lpKey == NULL || (!jvRoot.isMember(lpKey)))	return bDefault;
			if(!jvRoot[lpKey].isBool())	return bDefault;
			return jvRoot[lpKey].asBool();
		}
		catch(...) { return bDefault;}
	}

	static int64_t GetJsonValueInt64(const char* lpKey,const Json::Value& jvRoot, int64_t nDefault = 0)
	{
		try
		{
			if(jvRoot.isNull() || !jvRoot.isObject())		return nDefault;
			if(lpKey == NULL || (!jvRoot.isMember(lpKey)))	return nDefault;
			if(!jvRoot[lpKey].isInt64())	return nDefault;
			return jvRoot[lpKey].asInt64();
		}
		catch(...) { return nDefault;}
	}

	static float GetJsonValueFloat(const char* lpKey,const Json::Value& jvRoot, float nDefault = 0.0)
	{
		try
		{
			if(jvRoot.isNull() || !jvRoot.isObject())		return nDefault;
			if(lpKey == NULL || (!jvRoot.isMember(lpKey)))	return nDefault;
			if(!jvRoot[lpKey].isDouble())	return nDefault;
			return jvRoot[lpKey].asFloat();
		}
		catch(...) { return nDefault;}
	}
	...
	static bool WriteJsonValueBool(const char* lpKey, Json::Value& jvRoot, bool bValue)
	{
		try
		{
			if(lpKey == NULL || strlen(lpKey) <= 0)	return false;
	...
	static bool WriteJsonValueInt64(const char* lpKey, Json::Value& jvRoot, int64_t nValue)
	{
		try
		{
			if(lpKey == NULL || strlen(lpKey) <= 0)	return false;
			jvRoot[lpKey] = (Json::Int64)nValue;
			return true;
		}
		catch(...) { return false;}
	}
