...
std::string GetExecFileName(const std::string &proc_comm,
    ...
    if (split_stat.size() > 2 && split_stat[1].size() > 2) {
        std::string stat_name = split_stat[1].substr(1, split_stat[1].size() - 2);
        if (!stat_name.empty()) {
            return stat_name;
        }
    }

    // 若 ls -l /proc/$pid/exe -> /abc
    // 执行 mv /def /abc 进行更新
    // 则 ls -l /proc/$pid/exe -> /def (deleted)
    // exe 在这种场景下不准确，优先使用comm和stat
    std::string link;
    link = GetSymLinkTarget(proc_exe);
    if (link.size() > 0) {
...
std::string GetInstallPath() {
    std::string install_path = "/opt/qaxsafe/";
...
std::string GetAppPath() {
    std::string install_path = std::string("/opt/qaxsafe/qaxsafed"); // /opt/360safe/360entclient
    char szTemp[PATH_MAX] = {0};
    int nRet = readlink("/proc/self/exe", szTemp, PATH_MAX);
    if (nRet > 0 && nRet < PATH_MAX) {
        install_path = szTemp;
...
pid_t GetPid(const std::string &process_name) {
    std::string cmd = "pgrep " + process_name;
...
std::string GetCmdLine(int pid) {
    ...
    std::stringstream ss;
    ss << "/proc/" << pid << "/cmdline";
    std::ifstream cmdLineFile(ss.str().c_str());
    if (!cmdLineFile) {
        return cmdLine;
    }
    ...
    // Replace the null characters in the command line with spaces
    std::replace(cmdLine.begin(), cmdLine.end(), '\0', ' ');

    return cmdLine;
}

std::vector<int> GetPids(const std::string& processName) {
    std::vector<int> pids;
    ...
    while ((entry = readdir(dir)) != NULL) {
        // Check if the entry is a directory (corresponds to a process)
        ...
        std::string procName = GetExecFileName(pid);
...
bool IsZombieProcess(int pid) {
    ...
    char status[256] = {0};
    char state = 0;
    snprintf(stat_file, 256, "/proc/%d/stat", pid);
    FILE *fp = fopen(stat_file, "r");
    if (fp) {
        fscanf(fp, "%*d %s %c", status, &state);
        fclose(fp);
    }
    return state == 'Z';
}
