...
namespace f_type {
    FileType get_file_type(const std::string &path) {
        ...
        do {
            ...
            std::string s_Type, s_Mime;
            if (!get_file_type(path, s_Type, s_Mime))
                break;
            
            if (Map.find(s_Type) != Map.end()) {
                type = Map[s_Type];
            }
        } while (false);
        return type;
    ...
    bool get_file_type(const std::string &s_path, std::string& type, std::string& mime) {
        std::ifstream ifs(s_path.c_str(), std::ios::binary);
        if (ifs.is_open() == false) {
            return false;
        }

        std::string s_buf((std::istreambuf_iterator<char>(ifs)), (std::istreambuf_iterator<char>()));
        if (s_buf.length() <= 1) {
            return false;
        }
