...
namespace socket_control {
    ...
    int ConvertRecvStrToBundle(IASBundle** pSocketData, const std::string& recv_data) {
        ...
        do {
            ...
            if (str_content.length()) {
                ...
                LOG_DEBUG("convert recv str to bundle, str[%s], len[%d]", 
                        std::string((char *)recvData.lpContent, recvData.nContLen).c_str(), 
                        recvData.nContLen);
            } else {
            ...
            GetJsonItemInfo(json, UnixSocketKeyDataSenderUID, recvData.nSenderUid, cJSON_Number);
            GetJsonItemInfo(json, UnixSocketKeyDataRecverUID, recvData.nRecverUid, cJSON_Number);
            rtn = 0;
        } while(false);
        if (NULL != json) {
            cJSON_Delete(json);
        ...
        if (rtn == -1) {
            ...
            (*pSocketData)->putInt(UnixSocketKeyDataSenderUID, recvData.nSenderUid);
            (*pSocketData)->putInt(UnixSocketKeyDataRecverUID, recvData.nRecverUid);
        }
        recvData.clear();
        return rtn;
    }
    ...
    int GetBundleItemInfo(IASBundle* pBundle, const char *key, unsigned char * &value) {
        ...
        if((ASErr_INSUFFICIENT_BUFFER != pBundle->getBinary(key,NULL,&nInBufLen)) || nInBufLen <= 0) {
            LOG_DEBUG("get bundle info[%s] failed, because get binary length failed.", key);
            return -1;
        } else {
            value = new (std::nothrow) unsigned char[nInBufLen];
            if (!value) {
                LOG_DEBUG("get bundle info[%s] failed, because out of memory.", key);
                return -1;
            }
            memset(value, 0, nInBufLen);
            if((0 != pBundle->getBinary(key, value, &nInBufLen)) || nInBufLen <= 0) {
                LOG_DEBUG("get bundle info[%s] failed, because get binary buffer failed.", key);
                return -1;
            }
        }
        return 0;
    ...
    int GetBundleItemInfo(IASBundle* pBundle, const char *key, std::string &value) {
        ...
        if((ASErr_INSUFFICIENT_BUFFER != pBundle->getAString(key,NULL,&nInBufLen)) || nInBufLen <= 0) {
            LOG_DEBUG("get bundle info[%s] failed, because get string length failed.", key);
            return -1;
        } else {
            ...
            if((0 != pBundle->getAString(key, lpValue, &nInBufLen)) || nInBufLen <= 0) {
                LOG_DEBUG("get bundle info[%s] failed, because get string buffer failed.", key);
    ...
    int ParseRecvBundleData(IASBundle* pSocketData, struct UnixSocketData &recvData) {
        ...
        do {
            ...
            GetBundleItemInfo(pSocketData, UnixSocketKeyDataSenderUID, recvData.nSenderUid);
            GetBundleItemInfo(pSocketData, UnixSocketKeyDataRecverUID, recvData.nRecverUid);
            rtn = 0;
        } while(false);
        return rtn;
    }
    ...
    int CreateSendData(std::string& send_data, const struct UnixSocketData &recvData) {
        ...
        do {
            ...
            cJSON_AddNumberToObject(json, UnixSocketKeyDataSenderUID, recvData.nSenderUid);
            cJSON_AddNumberToObject(json, UnixSocketKeyDataRecverUID, recvData.nRecverUid);
    ...
    int ParseSendJsonData(const std::string& send_data, struct UnixSocketData& sendData) {
        ...
        do {
            ...
            if (str_content.length()) {
                ...
                LOG_DEBUG("convert recv str to bundle, str[%s], len[%d]", 
                    std::string((char *)sendData.lpContent, sendData.nContLen).c_str(), 
                    sendData.nContLen);
            } else {
                LOG_ERROR("convert recv data to bundle, recv data's content is NULL.");
                break;
            ...
            GetJsonItemInfo(json, UnixSocketKeyDataSenderUID, sendData.nSenderUid, cJSON_Number);
            GetJsonItemInfo(json, UnixSocketKeyDataRecverUID, sendData.nRecverUid, cJSON_Number);
            rtn = 0;
        } while(false);
    ...
    std::string getBundleBinaryInfo(IASBundle *pData, const char *str_key) {
        ...
        if (rtn == -1) {
            ...
            result = std::string((char *)lpValue, nInBufLen);
        }
        if (lpValue) {
            delete [] lpValue;
        }
    ...
    std::string GetBundleStringInfo(IASBundle* pData, const char* str_key) {
        ...
        if (rtn == -1) {
            ...
            result = std::string(lpValue, nInBufLen - 1);
        }
        if (lpValue) {
            delete [] lpValue;
        }
    ...
    bool GetJsonIntInfo(const std::string& json_data, const char * str_key, int &nValue) {
        ...
        cJSON* item = cJSON_GetObjectItem(json, str_key);
        if (item) {
            nValue = item->valueint;
        } else {
            LOG_DEBUG("parse send data error, get int[%s] failed.", str_key);
            bret = false;
        }
        if (json) cJSON_Delete(json);
        return bret;
    ...
    bool GetBundleItem2(IASBundle* pBundle, const char *key,u_char* &value)
    {
        ...
        do {
            int nInBufLen = 0;
            //预期as_rc会返回ASErr_NOBUF，nInBufLen返回值为大于0
            ASCode as_rc = pBundle->getBinary(key,
                                  NULL,&nInBufLen);
            if((nInBufLen <= 0) || 
              (as_rc != ASErr_NOBUF)) { break; }
            
            value = new (std::nothrow) u_char[nInBufLen]();
            if(!value) { break; }
            
            as_rc = pBundle->getBinary(key,value,&nInBufLen);
            ok = ((as_rc == ASErr_OK) && (nInBufLen > 0));
        } while(false);
    ...
    bool GetBundleItem2(IASBundle* pBundle,const char *key, std::string &value)
    {
        ...
        do {
            int nInBufLen = 0;
            ASCode as_rc = pBundle->getAString(key,
                                    NULL,&nInBufLen);
            if((nInBufLen <= 0) || 
              (as_rc != ASErr_NOBUF)) { break; }
            ...
            memset(lpValue, 0, nInBufLen);
            as_rc = pBundle->getAString(key,lpValue,&nInBufLen);
            ok = ((as_rc == ASErr_OK) && (nInBufLen > 0));
            if(ok) { value = std::string(lpValue); }
    ...
    bool GetBundleItem2(IASBundle* pBundle, const char *key, unsigned int &value) {
        ASCode as_rc = pBundle->getInt(key, (int *)&value);
        return (as_rc == ASErr_OK);
    }

    bool GetBundleItem2(IASBundle* pBundle, const char *key, int &value) {
        ASCode as_rc = pBundle->getInt(key, &value);
        return (as_rc == ASErr_OK);
    }
    bool GetBundleItem2(IASBundle* pBundle, const char *key, int64_t &value) {
        ASCode as_rc = pBundle->getInt64(key, &value);
        return (as_rc == ASErr_OK);
    }

    bool GetBundleItem2(IASBundle* pBundle, const char *key, bool &value) {
        int nVal;
        bool bret = (ASErr_OK == pBundle->getInt(key,&nVal));
        if(bret) { value = (nVal == 1 ? true : false); }
